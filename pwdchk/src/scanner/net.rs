pub mod net_ {
    ...
    async fn tcp_ping(host: &str, port: u16) -> Result<IdentificationResult, Error> {
        ...
        match test_connexion {
            Err(x) => Err(Error::Timeout(x)), //if timeout return Error type with elapsed timeout
            Ok(res) => match res {
                Err(_) => Ok(IdentificationResult::ConnexionRefused), //if connexion could not establish return ConnexionRefused
    ...
    async fn tcp_ping_many<'a>(
        ...
        res.collect::<Vec<_>>().await //collect tuple in a vector and wait for futures completion with await
    }
    ...
    pub fn expand_net(host: &str) -> Vec<String> {
        if host.contains('/') {
            ...
            net.hosts().map(|x| x.to_string()).collect::<Vec<String>>() //iteration on adresses which are converted into strings and collect
        } else {
    ...
    async fn welcome_line(stream: TcpStream) -> IdentificationResult {
        ...
        match test_welcome_line {
            Err(_) => IdentificationResult::NoWelcomeLine, //if timeout return NoWelcomeLine
            Ok(line) => match line {
                Ok(_) => return IdentificationResult::WelcomeLine(buf.strip_suffix('\n').unwrap().to_string()), //if a line is read return it in buf String
                Err(_) => IdentificationResult::NoWelcomeLine, //if an error occurs in read-line() function return NoWelcomeLine
            }
        }
    }
}
